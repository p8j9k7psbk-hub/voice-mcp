...
async function generateAudio(env: Env, text: string): Promise<{ success: boolean; audio_base64?: string; error?: string }> {
  try {
    ...
    const ttsUrl = `https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(env.ELEVENLABS_VOICE_ID)}?output_format=mp3_44100_128`;
    ...
    if (!response.ok) {
      ...
      return { success: false, error: `ElevenLabs ${response.status}: ${detail}` };
    }
    ...
    return { success: true, audio_base64: btoa(binary) };
  } catch (e) {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path === '/status') {
      return Response.json({
        ...
        service: 'voice-mcp',
        version: '1.0.0',
        voice_id: env.ELEVENLABS_VOICE_ID ? 'configured' : 'not configured',
      }, { headers: corsHeaders });
    }
...
<h1>🎙️ voice-mcp</h1>
<p>An MCP server for ElevenLabs voice synthesis with an inline audio player.</p>
